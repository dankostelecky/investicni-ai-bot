...
def render_user_manual():
    ...
    with st.expander("📖 1. How to Launch and Control the App"):
        st.markdown("The application runs directly in your web browser and is split into main sections via the top menu:")
        st.markdown("1. **📊 Market Scanning & Overview:** Scan markets, add custom tickers in the sidebar, use filters like **Gain ≥ 0.08 USD**, and run AI analysis.")
        st.markdown("2. **🧠 AI Accuracy & History (Backtesting):** Check historical predictions saved in the database.")
        st.markdown("3. **🤖 Klondike Agent Hub:** Monitor automated execution protocols and agent triggers.")
        st.markdown("4. **🏛️ Trump & Insider Trades:** Track political and insider transactions.")
                ...
                for ticker in active_tickers:
                    try:
                        ...
                        if vol_spike and flush_drop and is_recovering:
                            ...
                            capitulation_status = "⚠️ **Crowd Panic Warning:** High volume sell-off detected, stop-loss cascades might still be expanding."
                            capitulation_state_type = "warning"
                        else:
                            capitulation_status = "👥 **Crowd Behavior:** Normal volume patterns, no severe margin call flushes active."
                            capitulation_state_type = "info"
                        ...
                        with st.expander(f"Analysis for: {ticker} (Gain/USD: +{zisk_na_1_usd:.2f})"):
                            ...
                            if capitulation_state_type == "success":
                                st.success(capitulation_status)
                            elif capitulation_state_type == "warning":
                                st.warning(capitulation_status)
                            else:
                                st.info(capitulation_status)
